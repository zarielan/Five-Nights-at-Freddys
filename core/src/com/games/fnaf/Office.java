...
public class Office
{
	...
	public void render()
	{
		...
		FoxyAI.getInstance().setDoorShut(doorLights.isLeftDoor());
		FreddyAI.getInstance().setDoorShut(doorLights.isRightDoor());
	...
	private void renderDoors()
	{
		...
		if (playDoorAnimation[1])
		{
			...
			if (rightDoor.isAnimationFinished(doorAnimCounter[1]))
			{
				...
				batch.draw(rightDoor.getKeyFrame(doorAnimCounter[1]), 1120f, 0f);
		}
		...
		if (doorLights.isRightDoor() && !playDoorAnimation[1])
			batch.draw(Art.rightDoor.peek(), 1120f, 0f);
